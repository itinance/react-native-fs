...
- (NSString *)downloadFile:(RNFSDownloadParams *)params
{
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:_params.toFile]) {
    ...
    if (!_fileHandle) {
      NSError *error = [NSError errorWithDomain:@"Downloader"
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location
{
  ...
  if([_statusCode integerValue] >= 200 && [_statusCode integerValue] < 300) {
    [fileManager removeItemAtURL:destURL error:nil];       // Remove file at destination path, if it exists
    ...
    _bytesWritten = @([fileManager attributesOfItemAtPath:_params.toFile error:nil].fileSize);
  }
  if (error) {
